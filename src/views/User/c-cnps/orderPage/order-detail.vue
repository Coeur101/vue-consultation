...
import { reqMedicineOrderDetail } from '@/api/Medicine'
...
const { handleDelOrder } = useDelOrder(() => {
...
const loadOrder = async () => {
  const res = await reqMedicineOrderDetail(route.params.id as string)
...
<template>
  <div class="order-detail-page" v-if="order">
    ...
    <van-action-bar v-if="order.status === OrderType.MedicineCancel">
      <van-action-bar-icon
        icon="delete-o"
        @click="handleDelOrder(order)"
        text="删除"
      />
      <van-action-bar-button
        type="primary"
        :to="'/room?orderId=' + order.roomId"
        text="沟通记录"
      />
    </van-action-bar>
    <!-- 待收货 -->
